...
st.markdown("""
<style>
.stApp { background-color:#0b0c10 !important; color:#f5f5f7 !important;
    font-family:'Inter',system-ui,-apple-system,sans-serif; }
h1,h2,h3,h4,h5,h6 { color:#fff !important; font-weight:800 !important; letter-spacing:-0.7px; }
div[data-testid="column"] {
    background-color:#12131a; padding:18px !important; border-radius:12px;
    border:1px solid #1f222e; box-shadow:0 8px 32px rgba(0,0,0,0.4); margin-bottom:20px;
}
.stSelectbox div[data-baseweb="select"], .stTextInput input, .stNumberInput input {
    background-color:#1a1c23 !important; color:#fff !important;
    border:1px solid #2f3346 !important; border-radius:8px !important;
}
.stTextArea textarea {
    background-color:#090a0f !important; color:#e4e6eb !important;
    border:1px solid #2f3346 !important; border-radius:6px !important;
    font-size:13.5px !important; padding:10px !important;
}
.stTextArea textarea:focus {
    border-color:#00e676 !important; box-shadow:0 0 8px rgba(0,230,118,0.2) !important;
}
label[data-testid="stWidgetLabel"] {
    color:#8a90a6 !important; font-weight:600 !important; font-size:13.5px !important;
}
div[data-testid="stExpander"] {
    background-color:#12131a !important; border:1px solid #252836 !important;
    border-radius:8px !important; margin-bottom:10px !important; overflow:hidden !important;
}
div[data-testid="stExpander"] details summary {
    background-color:#1a1c23 !important; color:#fff !important;
    border-bottom:1px solid #252836 !important; padding:12px 15px !important;
}
...
div[data-testid="stExpander"] [role="transition-container"] {
    background-color:#12131a !important; padding:15px !important;
}
div[data-testid="stExpander"] svg { fill:#00e676 !important; color:#00e676 !important; }
button[data-baseweb="tab"] {
    color:#8a90a6 !important; font-size:14.5px !important; font-weight:800 !important;
    padding:12px 18px !important; background-color:transparent !important; border:none !important;
}
button[data-baseweb="tab"][aria-selected="true"] {
    color:#00e676 !important; border-bottom:3px solid #00e676 !important;
}
div.stButton > button[kind="secondary"], div.stButton > button:not([kind]) {
    ...
    color:#fff !important; border:1px solid #3a3f58 !important; border-radius:8px !important;
    padding:12px 24px !important; font-weight:700 !important; font-size:14px !important;
    text-transform:uppercase; letter-spacing:0.8px; width:100%; transition:all .2s ease !important;
}
div.stButton > button[kind="secondary"]:hover, div.stButton > button:not([kind]):hover {
    ...
    box-shadow:0 0 20px rgba(0,230,118,0.3) !important; transform:translateY(-1.5px);
}
div.stButton > button[kind="primary"] {
    background:linear-gradient(135deg,#2a0d12 0%,#3a1218 100%) !important;
    color:#ff5a6c !important; border:1px solid #ff1744 !important; border-radius:8px !important;
    padding:12px 24px !important; font-weight:700 !important;
    text-transform:uppercase; letter-spacing:0.8px; width:100%;
}
div.stButton > button[kind="primary"]:hover {
    color:#ff1744 !important; box-shadow:0 0 20px rgba(255,23,68,0.35) !important;
}
div[data-testid="stCodeBlock"] {
    background-color:#07080a !important; border:1px solid #1a1c23 !important; border-radius:8px !important;
}
.inactive-badge {
    display:inline-block; padding:2px 8px; border-radius:4px;
    background:#2a2d3a; color:#8a90a6; font-size:11px; font-weight:600; letter-spacing:0.5px;
}
.active-badge {
    display:inline-block; padding:2px 8px; border-radius:4px;
    background:#0a3a1c; color:#00e676; font-size:11px; font-weight:600; letter-spacing:0.5px;
}
</style>
...
TRANSLATIONS = {
    "English": {
        ...
        "prompt_cap":"Empty prompt = player is inactive (stands still, transparent on pitch).",
        "btn_reset":"🔄 RESET BALL","debug_hdr":"Debug Console","sim_title":"Live Simulation",
        "time":"Time","live":"Live Match","strat_hdr":"Live Synced Strategies",
    ...
    "Deutsch": {
        ...
        "prompt_hdr":"Live Prompt Engineering",
        "prompt_cap":"Leerer Prompt = Spieler inaktiv (steht still, transparent dargestellt).",
        "btn_reset":"🔄 BALL ZURÜCKSETZEN","debug_hdr":"Debug-Konsole","sim_title":"Live-Simulation",
        "time":"Zeit","live":"Live-Match","strat_hdr":"Aktive Strategien",
        "setup_hdr":"🏆 Match-Setup",
    ...
    "Français": {
        ...
        "prompt_cap":"Prompt vide = joueur inactif (immobile, transparent sur le terrain).",
        "btn_reset":"🔄 RÉINITIALISER LE BALLON","debug_hdr":"Console de Débogage",
        "sim_title":"Simulation en Direct","time":"Temps","live":"Match en Direct",
    ...
    "Español": {
        ...
        "prompt_cap":"Prompt vacío = jugador inactivo (inmóvil, transparente en el campo).",
        "btn_reset":"🔄 REINICIAR BALÓN","debug_hdr":"Consola de Depuración",
        "sim_title":"Simulación en Vivo","time":"Tiempo","live":"Partido en Vivo",
    ...
    "Polski": {
        ...
        "prompt_cap":"Pusty prompt = gracz nieaktywny (stoi w miejscu, przezroczysty).",
        "btn_reset":"🔄 RESETUJ PIŁKĘ","debug_hdr":"Konsola Debugowania",
        "sim_title":"Symulacja na Żywo","time":"Czas","live":"Mecz na Żywo",
        ...
        "lbl_active":"AKTYWNY","lbl_inactive":"NIEAKTYWNY",
    }
}
...
# =====================================================================
# 5. SPIELFELD-KONSTANTEN (800x500)
# =====================================================================
FIELD_W   = 800
FIELD_H   = 500
GOAL_Y_MIN = 175
GOAL_Y_MAX = 325
BALL_START = (FIELD_W // 2, FIELD_H // 2)
...
def make_default_prompts(name_red, name_blue):
    return {
        ...
        "red_midfielder":  f"You are the {name_red} Midfielder. Distribute the ball to the striker and support from behind.",
        ...
        "blue_midfielder": f"You are the {name_blue} Midfielder. Stop {name_red} attacks and pass the ball to the {name_blue} striker.",
        "blue_defender":   f"You are the {name_blue} Defender. Guard the right goal area and block {name_red} attackers.",
    }
...
def get_shared_match():
    ...
    return {
        ...
        "last_llm_call": 0.0,
        "api_error": None,
        "ball": {"x": BALL_START[0], "y": BALL_START[1], "vx": 0, "vy": 0},
        "ball_trail": [],
        "team_red_name": name_red, "team_red_nation":"Switzerland", "team_red_color":"#ff1744",
        "team_blue_name":name_blue,"team_blue_nation":"Germany",   "team_blue_color":"#00b0ff",
...
def apply_match_setup(name_red, nation_red, color_red, name_blue, nation_blue, color_blue, duration):
    ...
    shared_state["ball"]             = {"x": BALL_START[0], "y": BALL_START[1], "vx": 0, "vy": 0}
...
def is_active(p):
    return bool(p.get("prompt", "").strip())
 
# =====================================================================
# 8. LLM – filtert inaktive Spieler raus
# =====================================================================
def fetch_all_agent_moves(current_state):
    name_red  = current_state["team_red_name"]
    name_blue = current_state["team_blue_name"]
    active = {n: p for n, p in current_state["players"].items() if is_active(p)}
    ...
    system = (
        ...
        f"Field {FIELD_W}x{FIELD_H}. Red team = '{name_red}' attacks RIGHT goal (x={FIELD_W}). "
    ...
    try:
        ...
        # Inaktive Spieler explizit auf 0/false setzen
        for n in current_state["players"].keys():
            if n not in parsed:
...
def run_game_tick():
    ...
    if now - shared_state["last_llm_call"] > LLM_THROTTLE:
        ...
        for name, p in shared_state["players"].items():
            if not is_active(p):
                p["dx"] = 0.0; p["dy"] = 0.0; p["kick"] = False
                continue
            mv = moves.get(name, {})
    ...
    for p in shared_state["players"].values():
        if not is_active(p):
            continue
        p["x"] = max(20, min(FIELD_W - 20, p["x"] + p["dx"]))
        p["y"] = max(20, min(FIELD_H - 20, p["y"] + p["dy"]))
 
        dx = p["x"] - shared_state["ball"]["x"]
        dy = p["y"] - shared_state["ball"]["y"]
        if dx*dx + dy*dy < 625 and p["kick"]:
            direction = 13 if p["team"] == "Red" else -13
    ...
    if len(shared_state["ball_trail"]) > 14:
        shared_state["ball_trail"].pop(0)
 
    if b["x"] > FIELD_W - 20 and GOAL_Y_MIN < b["y"] < GOAL_Y_MAX:
        shared_state["score"]["Red"] += 1
        reset_ball()
    elif b["x"] < 20 and GOAL_Y_MIN < b["y"] < GOAL_Y_MAX:
        shared_state["score"]["Blue"] += 1
        reset_ball()
    else:
        if b["x"] < 8 or b["x"] > FIELD_W - 8:
            b["vx"] *= -0.5
            b["x"] = max(8, min(FIELD_W - 8, b["x"]))
        if b["y"] < 8 or b["y"] > FIELD_H - 8:
            b["vy"] *= -0.5
            b["y"] = max(8, min(FIELD_H - 8, b["y"]))
 
    shared_state["time_left"] = max(0.0, shared_state["time_left"] - TICK_INTERVAL)
 
# =====================================================================
# 10. PITCH RENDER (800x500, volles Feld, inaktive Spieler transparent)
# =====================================================================
def generate_pitch_html(state, t):
    # active flag pro Player für JS
    players_payload = {
        name: {**p, "active": is_active(p)}
...
.container {{ background:#111215; padding:12px; border-radius:10px; color:white;
...
<div class="container">
  ...
  <canvas id="field" width="{FIELD_W}" height="{FIELD_H}"
...
const GMIN = {GOAL_Y_MIN}, GMAX = {GOAL_Y_MAX};
 
// Spielfeldlinien
ctx.strokeStyle = "rgba(255,255,255,0.8)"; ctx.lineWidth = 2.5;
ctx.beginPath(); ctx.moveTo(W/2, 0); ctx.lineTo(W/2, H); ctx.stroke();
ctx.beginPath(); ctx.arc(W/2, H/2, 60, 0, 2*Math.PI); ctx.stroke();
ctx.beginPath(); ctx.arc(W/2, H/2, 3, 0, 2*Math.PI); ctx.fillStyle = "rgba(255,255,255,0.8)"; ctx.fill();
 
// Strafraum links + rechts
ctx.strokeStyle = "rgba(255,255,255,0.8)";
ctx.strokeRect(0, 125, 100, 250);
ctx.strokeRect(W-100, 125, 100, 250);
 
// Tor-Visualisierung
ctx.fillStyle = "rgba(255,255,255,0.22)";
ctx.fillRect(0, GMIN, 10, GMAX-GMIN);
ctx.fillRect(W-10, GMIN, 10, GMAX-GMIN);
 
// Watermark mittig
ctx.fillStyle = "rgba(255,255,255,0.13)";
ctx.font = "bold 30px 'Impact',sans-serif"; ctx.textAlign = "center";
ctx.fillText("MAKE FOOTBALL TEAM", W/2, H/2 + 10);
 
// Ball-Trail
for (let i = 0; i < trail.length; i++) {{
  const alpha = (i + 1) / trail.length * 0.5;
  ctx.beginPath();
  ctx.arc(trail[i][0], trail[i][1], 3 + i * 0.35, 0, 2*Math.PI);
  ctx.fillStyle = `rgba(255,255,255,${{alpha}})`;
  ctx.fill();
}}
 
// Ball
ctx.beginPath(); ctx.arc(ball.x, ball.y, 8, 0, 2*Math.PI);
...
for (let name in players) {{
  ...
  const baseColor = p.team === "Red" ? RED_COLOR : BLUE_COLOR;
...
with col_left:
    ...
    with st.expander(lang["setup_hdr"], expanded=False):
        ...
        with c1:
            in_name_red = st.text_input(lang["f_name"], value=shared_state["team_red_name"], key="su_name_red")
        with c2:
        ...
        with c3:
            in_color_red = st.color_picker(lang["f_color"], value=shared_state["team_red_color"], key="su_color_red")
 
        st.markdown(f"**{lang['team_b']}**")
        c4, c5, c6 = st.columns([1.3, 1, 0.6])
        with c4:
            in_name_blue = st.text_input(lang["f_name"], value=shared_state["team_blue_name"], key="su_name_blue")
        with c5:
        ...
        with c6:
            in_color_blue = st.color_picker(lang["f_color"], value=shared_state["team_blue_color"], key="su_color_blue")
 
        in_duration = st.slider(
            lang["f_duration"], min_value=30, max_value=300,
            value=int(shared_state["match_duration"]), step=10, key="su_duration"
        )
    ...
    def render_team_prompts(team_label, flag):
        for name, p in shared_state["players"].items():
            if p["team"] != team_label:
                continue
            role_display = name.split('_')[1].upper()
            badge_cls = "active-badge" if is_active(p) else "inactive-badge"
            badge_lbl = lang["lbl_active"] if is_active(p) else lang["lbl_inactive"]
            header    = f"{flag} {role_display}"
            with st.expander(header, expanded=(role_display == "STRIKER")):
                st.markdown(
                ...
                shared_state["players"][name]["prompt"] = st.text_area(
                    ...
                    key=f"input_{name}", height=80, label_visibility="collapsed"
                )
 
    with tab_red:
        render_team_prompts("Red", red_flag)
    with tab_blue:
        render_team_prompts("Blue", blue_flag)
    ...
    with st.expander("📋 " + lang["strat_hdr"], expanded=False):
        st.json({
            name.upper(): {
                ...
                "Status": lang["lbl_active"] if is_active(data) else lang["lbl_inactive"],
                "Strategy": data["prompt"] or "—",
            }
...
with col_right:
    ...
    components.html(html_pitch, height=560, scrolling=False)
