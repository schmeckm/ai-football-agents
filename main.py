...
async def get_moves(req: MoveRequest):
    ...
    system = (
        ...
        '* Midfielder at (200,300), ball at (400,200) -> {"x":4,"y":-3,"k":false}  (always chase the ball)\n'
        '* Defender at (100,250), ball at (600,250) -> {"x":-1,"y":0,"k":false}  (ball far away, hold position)\n\n'
        "IMPORTANT: Prefer movement over standing still. Almost always set x and y based on where the ball is relative to the player. Only output 0 for x/y if the strategy explicitly says to hold position AND the ball is far away.\n\n"
        "PLAYER STRATEGIES:\n" + strategies
    )
